...
public class AES {
  ...
  public static void main(String[] args) {
    ...
    int[] bytes =
    {0xB4,0x69,0x69,0x3D,0x49,0x17,0x17,0x9B,0xB4,0x69,0x69,0x3D,0x49,0x17,0x17,0x9B};

    System.out.println("after sub bytes: ");
    int[][] state = new int[4][Nb];
    makeState(bytes, state);
    printState(state);
    ...
    mixColumns(state);
    System.out.println("after mix mixColumns: ");
    printState(state);

    int[] res_bytes = new int[bytes.length];
    //get the bytes from the state
    stateToBytes(state, res_bytes);
  ...
  static void mixColumns(int[][] arr) {
    int[][] temp_state = new int[4][4];
    for(int i = 0; i < 4; i++)
  ...
  static int mult(int[][] arr, int i, int j){
    int sum = 0;
    for (int k = 0; k < 4; k++) {
      int a = galois[i][k];
      int b = arr[k][j];
